...
**Update, same day — fix applied and re-verified (`860975e`):** tolerant key implemented in `pickGrounding.ts` as proposed (Unicode-folded, title up to first colon/parenthesis, empty title/author never matches; also keeps letters of any script). 22 unit checks pass (the real failing strings, plus guards that the earlier truly-ungrounded picks are still rejected). Live re-run of cases 3, 4, 1, 8: all HTTP 200 on attempt 1, all picks OL-sourced, no retries; cases 4 and 8 each picked *Sátántangó* (model precomposed vs pool decomposed) — the old key would have rejected both. (A first attempt at this re-run was invalid — Open Library was unreachable and every pool was empty — and was redone after recovery.) The other 7 cases were not re-run after the fix. Raw: `scratchpad/eval-run-grounded-fix-verify-2026-09-20.json`. PR #5 is ready for review; merge is a separate decision.
